...
public class RenderBlockDerrick extends RenderBlockMulti {
	public static final RenderBlockDerrick INSTANCE = new RenderBlockDerrick();
	...
	public void renderOnInventory(Block block, int metadata, float brightness, RenderBlocks renderer) {
		...
		mc.renderEngine.bindTexture(mc.renderEngine.getTexture(OBJInit.oil_derrick_tex));
		GL11.glPushMatrix();
		GL11.glDisable(GL11.GL_CULL_FACE);
		final float scale = 1.0F / 3.5F;
		GL11.glTranslatef(0.0F, -0.5F, 0.0F);
		GL11.glScalef(scale, scale, scale);
		OBJInit.oil_derrick.callAllLists();
